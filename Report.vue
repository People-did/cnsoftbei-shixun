<template>
  <div class="page-container">
    <h2 class="page-title">{{ isTeacher ? '报告与评价' : '我的实训报告' }}</h2>
      ...
      <el-table v-else :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column label="状态" width="90" align="center">
          <template #default="{ row }">
            <el-tag v-if="isTeacher ? (row.reportStatus === 1) : (row.status === 1)" type="success" size="small">已评价</el-tag>
        ...
        <template v-if="isTeacher">
          <el-table-column label="完整性" width="85" align="center">
            <template #default="{ row }">
              <span v-if="row.reportStatus === 1 && !row.dynamicScoresJson">{{ row.completenessScore }}</span>
              <span v-else class="text-muted">-</span>
            </template>
          </el-table-column>
          <el-table-column label="规范性" width="85" align="center">
            <template #default="{ row }">
              <span v-if="row.reportStatus === 1 && !row.dynamicScoresJson">{{ row.specificationScore }}</span>
              <span v-else class="text-muted">-</span>
            </template>
          </el-table-column>
          <el-table-column label="知识点" width="85" align="center">
            <template #default="{ row }">
              <span v-if="row.reportStatus === 1 && !row.dynamicScoresJson">{{ row.knowledgeScore }}</span>
              <span v-else class="text-muted">-</span>
            </template>
          </el-table-column>
        </template>
        ...
        <el-table-column label="综合总分" width="95" align="center">
          <template #default="{ row }">
            <el-tag v-if="isTeacher ? (row.reportStatus === 1) : (row.totalScore)" :type="getScoreType(row.totalScore)" size="small">
              {{ row.totalScore }}分
            </el-tag>
            <span v-else class="text-muted">-</span>
          </template>
        ...
        <el-table-column v-if="isTeacher" label="评价方式" width="100" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.reportStatus === 1" :type="row.isAi === 1 ? 'primary' : 'info'" size="small">
        ...
        <!-- 操作列 -->
        <el-table-column label="操作" :width="isTeacher ? 360 : 220" align="center" fixed="right">
          <template #default="{ row }">
      ...
      <div v-if="currentDetail" v-loading="detailLoading">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="状态">
            <el-tag :type="(currentDetail.reportStatus === 1 || currentDetail.status === 1) ? 'success' : 'warning'">
              {{ (currentDetail.reportStatus === 1 || currentDetail.status === 1) ? '已评价' : '待评价' }}
            </el-tag>
        ...
        <el-form label-width="100px">
          ...
          <el-form-item v-if="aiForm.rubricMode === 'custom'" label="评分标准">
            <el-input v-model="aiForm.customRubric" type="textarea" :rows="5"
              placeholder='[{"name":"完整性","weight":35,"maxScore":35,"description":"..."}]' />
          </el-form-item>
        </el-form>
              ...
              <el-table :data="fullReport.scoringDetails" border size="small">
                ...
                <el-table-column prop="score" label="得分" width="70">
                  <template #default="{ row: r }">
                    <el-tag :type="getScoreTagType(r.score, r.maxScore)">{{ r.score }}</el-tag>
                  </template>
                </el-table-column>
    ...
    <!-- 悬浮按钮：AI智能评价 -->
    <el-button v-if="isTeacher" type="success" class="ai-float-btn" @click="aiDialogVisible = true">
...
import { EditPen, Compass, ChatLineSquare, Opportunity, Loading, MagicStick } from '@element-plus/icons-vue'
...
const parsedDynamicScores = computed(() => {
  ...
  try {
    const obj = JSON.parse(currentDetail.value.dynamicScoresJson)
    return obj.scores || null
  } catch {
    try {
      const obj = JSON.parse(currentDetail.value.aiEvaluation)
      return obj.scores || null
    } catch { return null }
  }
})
  ...
  try {
    if (isTeacher.value) {
      ...
      if (params.status != null && params.status !== '') {
        params.reportStatus = params.status
      }
...
const handleStartAiEvaluate = async () => {
  ...
  if (aiForm.rubricMode === 'auto' && aiForm.rubricPreview.length > 0) {
    ...
    if (!aiForm.customRubric.trim()) { ElMessage.warning('请输入自定义评分标准'); return }
...
const finishAiEvaluate = () => {
  ...
  aiForm.rubricPreview = []; aiForm.customRubric = ''; aiForm.rubricMode = 'auto'
